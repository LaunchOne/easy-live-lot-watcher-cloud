# Easy Live Lot Watcher — version 0.7.4
...
Version 0.7.4 prevents an ended individual timed lot from closing the entire catalogue. Completed and manually removed account watches are remembered locally so automatic importing cannot add them back, and their processed alert stages remain de-duplicated. Railway v1.1.5 also recovers a live feed that stops reporting its current lot after the auction starts.
...
Version 0.7.1 fixes Wellers live sales whose catalogue does not render a visible **Bid Live** link. The extension and Railway now derive the matching live page from the catalogue's unique sale route, so an active sale displays its current lot and accurate distance to each watched lot. It also removes an unsafe identity fallback based on Wellers' reused auction-house identifier, preventing current watches from attaching to historical sales. Railway v1.1.1 retires live-sale records more than 72 hours after their advertised start and the popup uses the fresh Railway update time when cloud monitoring is active.
...
Replace the files inside the same previously loaded `easy-live-lot-watcher` folder with the version 0.7.4 files. Open `chrome://extensions`, click **Reload** on the extension, and reload auction tabs that were already open.
...
SMS alerts and automated bidding are not included in version 0.7.4.
