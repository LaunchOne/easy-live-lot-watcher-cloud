...
const elements = Object.fromEntries([
  ...
  "cloudStatusBar", "cloudStatusLabel", "cloudLastUpdate", "refreshCloud",
  "reconciliationDisclosure", "reconciliationSummary", "reconciliationTotals", "reconciliationList", "reconcileNow",
  "cloudAuditDisclosure", "cloudAuditSummary", "cloudAuditList", "cloudAuditEmpty"
...
function renderStatus(status) {
  ...
  const activeLive = status.mode === "live" && !scheduledLive && Boolean(status.currentLot);
  ...
  setBadge(
    ...
    complete ? status.auctionEnded ? "Auction ended" : "Complete"
      : activeLive ? `Live · lot ${status.currentLot}`
  ...
  elements.healthText.textContent = complete
    ...
    : activeLive ? `Live auction connected — current lot ${status.currentLot}`
...
async function refreshStatus() {
  ...
  if (response?.ok && response.status) renderStatus(await getEffectiveStatus(response.status));
}
...
function createDashboardCard(auction) {
  ...
  heartbeat.textContent = auction.monitoringComplete
    ...
    : auction.mode === "live" && auction.currentLot
      ? `Current lot ${auction.currentLot} · ${auction.cloudManaged ? "Railway" : "page"} updated ${formatLastSeen(auction.lastSeen)}`
...
function formatCloudTime(value) {
  if (!value) return "No cloud update yet";
  const elapsed = Math.max(0, Date.now() - Number(value));
  if (elapsed < 60000) return `Updated ${Math.round(elapsed / 1000)}s ago`;
  if (elapsed < 60 * 60000) return `Updated ${Math.round(elapsed / 60000)}m ago`;
  return `Updated ${new Date(value).toLocaleString()}`;
}

function renderCloudStatus(cloud) {
  elements.cloudStatusBar.classList.toggle("hidden", !cloud?.enabled);
  if (!cloud?.enabled) return;
  elements.cloudStatusBar.classList.toggle("disconnected", !cloud.connected);
  elements.cloudStatusLabel.textContent = cloud.connected ? `Railway connected${cloud.serviceVersion ? ` · v${cloud.serviceVersion}` : ""}` : "Railway needs attention";
  elements.cloudLastUpdate.textContent = cloud.connected ? formatCloudTime(cloud.checkedAt) : cloud.error || formatCloudTime(cloud.checkedAt);
}

function renderReconciliation(reconciliation) {
  elements.reconciliationDisclosure.classList.toggle("hidden", !reconciliation?.enabled);
  if (!reconciliation?.enabled) return;
  const local = Number(reconciliation.totals?.local || 0);
  const cloud = Number(reconciliation.totals?.cloud || 0);
  elements.reconciliationSummary.textContent = reconciliation.connected
    ? reconciliation.matched ? `${local} local · ${cloud} cloud · matched` : `${local} local · ${cloud} cloud · sync needed`
    : `${local} local · cloud unavailable`;
  elements.reconciliationTotals.replaceChildren();
  for (const [label, value] of [["Local active lots", local], ["Cloud active lots", cloud]]) {
    const box = document.createElement("span");
    const count = document.createElement("strong");
    count.textContent = String(value);
    const copy = document.createElement("small");
    copy.textContent = label;
    box.append(count, copy);
    elements.reconciliationTotals.append(box);
  }
  elements.reconciliationList.replaceChildren();
  for (const auction of reconciliation.auctions || []) {
    const row = document.createElement("div");
    row.className = `reconciliation-row ${auction.matched ? "matched" : "attention"}`;
    const copy = document.createElement("span");
    const title = document.createElement("strong");
    title.textContent = auction.label;
    const detail = document.createElement("small");
    const differences = [
      auction.missingInCloud?.length ? `${auction.missingInCloud.length} missing from cloud` : "",
      auction.cloudOnly?.length ? `${auction.cloudOnly.length} cloud-only` : ""
    ].filter(Boolean).join(" · ");
    detail.textContent = `Local ${auction.localCount} · Cloud ${auction.cloudCount}${differences ? ` · ${differences}` : ""}`;
    copy.append(title, detail);
    const badge = document.createElement("span");
    setBadge(badge, auction.matched ? "Matched" : "Sync needed", auction.matched ? "success" : "warning");
    row.append(copy, badge);
    elements.reconciliationList.append(row);
  }
}

function renderCloudAudit(events, enabled) {
  elements.cloudAuditDisclosure.classList.toggle("hidden", !enabled);
  if (!enabled) return;
  elements.cloudAuditList.replaceChildren();
  elements.cloudAuditEmpty.classList.toggle("hidden", events.length > 0);
  elements.cloudAuditSummary.textContent = events.length
    ? `${events.length} recent Railway delivery event${events.length === 1 ? "" : "s"}`
    : "Railway Pushover delivery audit";
  for (const event of events.slice(0, 20)) {
    const details = event.details || {};
    const row = document.createElement("div");
    row.className = `history-item ${event.level === "error" || details.pushoverAccepted === false ? "warning" : ""}`;
    const mark = document.createElement("span");
    mark.className = "history-mark";
    const content = document.createElement("div");
    const title = document.createElement("strong");
    if (event.type === "alert-sent" || event.type === "alert-failed") {
      const stage = details.mode === "timed" ? stageLabel("timed", Number(details.stage)) : stageLabel("live", Number(details.stage));
      title.textContent = `Lot ${details.lot || "—"} · ${stage} alert`;
    } else if (event.type.startsWith("pre-auction-warning")) {
      title.textContent = "Pre-auction live-feed warning";
    } else title.textContent = "Cloud alert event";
    const detail = document.createElement("small");
    const accepted = details.pushoverAccepted === true || event.type === "alert-sent";
    const delivery = accepted ? "Pushover accepted" : event.type.endsWith("failed") ? `Failed${details.error ? `: ${details.error}` : ""}` : "Recorded";
    detail.textContent = `${new Date(event.time).toLocaleString()} · ${delivery}${details.pushoverRequest ? ` · ID ${details.pushoverRequest}` : ""}`;
    content.append(title, detail);
    row.append(mark, content);
    elements.cloudAuditList.append(row);
  }
}
...
async function forceCloudRefresh(button) {
  ...
  try {
    const response = await chrome.runtime.sendMessage({ type: "SYNC_CLOUD" });
    if (!response?.ok) throw new Error(response?.error || "Cloud synchronization failed.");
    await refreshDashboard();
  } catch (error) {
    elements.cloudStatusLabel.textContent = "Railway needs attention";
    elements.cloudLastUpdate.textContent = error.message || "Cloud synchronization failed.";
  } finally {
...
async function refreshDashboard() {
  ...
  if (!pageStatus) {
    ...
    else if (active.some((auction) => !auction.connected || auction.healthProblem)) setReadinessIndicator({ state: "attention", label: "Attention", detail: "One or more watched auctions need attention" });
...
elements.refreshCloud.addEventListener("click", () => forceCloudRefresh(elements.refreshCloud));
elements.reconcileNow.addEventListener("click", () => forceCloudRefresh(elements.reconcileNow));
