...
fields.testCloud.addEventListener("click", async () => {
  ...
  try {
    ...
    setCloudStatus("Connected — test sent", "connected");
    setStatus("Railway received your watch list and sent a Pushover test alert.");
  } catch (error) {
...
(async () => {
  ...
  if (settings.cloudEnabled) {
    ...
    if (response?.status?.connected) setCloudStatus("Connected", "connected");
